...
async def list_agent_tasks(workspace_id: str, status: str | None = None, limit: int = 50) -> list[AgentTaskSchema]:
        ...
        while True:
            ...
            if action == "execute":
                ...
                async for event in executor.stream_execute(task_type, goal):
                    await websocket.send_text(json.dumps({"type": event["type"], "data": event.get("data", {})}))
                ...
                await websocket.send_text(json.dumps({"type": "cancelled", "data": {}}))
...
async def get_workspace_memory(workspace_id: str, query: str | None = None, limit: int = 20) -> dict:
    ...
    if query:
        ...
        return {"type": "search", "results": [{"key": m.memory_key, "content": m.content, "access_count": m.access_count} for m in results]}
...
async def agent_chat(request: ChatCompletionRequest) -> dict:
    ...
    if request.stream:
        return {"streaming": True, "model": request.model}
    result = await model_router.chat_completion(req=request)
    return result
    ...
    try:
        ...
        async for chunk in model_router.stream_completion(request):
            try:
                ...
                content = chunk[:500]
            await websocket.send_text(json.dumps({"type": "chunk", "id": response_id, "content": content}))

        await websocket.send_text(json.dumps({"type": "complete", "id": response_id}))
    except WebSocketDisconnect:
        pass
    except Exception as e:
