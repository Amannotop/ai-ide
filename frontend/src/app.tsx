  ...
  return (
    <div className="h-screen w-screen flex flex-col overflow-hidden bg-[#0e0e14] text-[#c8c8d4]">
      ...
      <div className="flex flex-1 overflow-hidden">
        <aside
          className={`transition-all duration-150 ease-in-out overflow-hidden border-r border-[#2a2a35] flex-shrink-0 ${sidebarOpen ? 'w-[260px]' : 'w-[48px]'}`}
        >
          <div className="h-full flex flex-col">
            <div className="px-3 py-2 border-b border-[#2a2a35] flex items-center justify-between min-h-[28px]">
              ...
              {sidebarOpen && (
                <button className="text-[#686880] hover:text-[#c8c8d4]" onClick={handleRefresh} title="Refresh">
            ...
            {sidebarOpen && (
              <div className="flex-1 overflow-y-auto">
                <FileExplorer data={fileExplorerData} onFileClick={handleFileOpen} />
              </div>
            )}
          </div>
        </aside>
        <main className="flex-1 flex flex-col overflow-hidden">
          <div className="flex-shrink-0 bg-[#18181e] border-b border-[#2a2a35] flex items-center overflow-x-auto h-[34px]">
            {tabs.map((tab) => (
              <div
                key={tab.id}
                className={`flex items-center gap-1.5 px-3 h-full cursor-pointer border-r border-[#2a2a35] text-xs min-w-[120px] max-w-[200px] truncate ${tab.id === activeTabId ? 'bg-[#0e0e14] text-[#c8c8d4] border-b-2 border-b-[#8888a0]' : 'bg-[#18181e] text-[#686880] hover:bg-[#1e1e28]'}`}
          ...
          <div className="flex-1 overflow-hidden">
            {activeTab ? <Editor tab={activeTab} /> : (
          ...
          {terminalPanelOpen && <TerminalPanel onClose={() => setTerminalPanelOpen(false)} />}
        </main>
        <div className={`flex-shrink-0 border-l border-[#2a2a35] flex flex-col overflow-hidden transition-all duration-150 ease-in-out ${aiPanelOpen ? 'w-[400px]' : 'w-[40px]'}`}>
          {aiPanelOpen && <AIChatPanel onClose={() => setAiPanelOpen(false)} />}
          {agentPanelOpen && <AgentPanel onClose={() => setAgentPanelOpen(false)} />}
        </div>
      </div>
      <StatusBar connected={connected} workspace={workspace} gitStatus={gitStatus} onOpenCommandPalette={() => setCommandPaletteOpen(true)} />
      <CommandPalette isOpen={commandPaletteOpen} onClose={() => setCommandPaletteOpen(false)} onOpenTerminal={() => setTerminalPanelOpen(true)} />
    </div>
  )
}
...
function getLanguageFromPath(path: string): string {
  ...
  const map: Record<string, string> = {
    ...
    '.dockerfile': 'dockerfile', '.env': 'properties', '.xml': 'xml', '.svg': 'xml',
  }
